...
def mirrorX(pattern):
    turned = []
    for point in pattern:
        newpoint = dict(xstep = -point['xstep'],
                        ystep = point['ystep'])
        turned.append(newpoint)
    return turned
...
B1 = {
        'pattern' : section1,
        #'pattern' : (mirrorX(mirrorY(section1))),
        'startX'  : 31,
        'startY'  : 67
}
...
#sections = [
#    C3, C2, B2, B3, A3, A2, B1, A1, C1, A4, B4, C4, D1, D2, D3, D4, X
#]
sections = [
    B2,C2,C3,B3,A3,C4,D3,D4,D2,A4,B4,A2,D1,C1,B1,A1,
]
